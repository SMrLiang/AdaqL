...
# 配置自己的 handler
file_handler = logging.FileHandler("../logs/ph1.txt")
...
# 数据构造
def get_epoch_batches_cali(tokenizer, device, dataset, micro_batch_size=4, shuffle=True, drop_last=False, seed=42):
...
def main():
    ...
    val_dataset = load_dataset("piqa", split="validation")
    # 1600
    ...
    for epoch in range(num_epoches):
        ...
        if epoch <= 8:
            ...
            bit_used = True
        for step, batch in tqdm(enumerate(get_epoch_batches_cali(tokenizer, model_engine.device, train_dataset, micro_batch_size, seed)), desc="training", leave=False):
            if(step % 100 == 0):
                step_print = True
            else:
                step_print = False
        ...
        grad_print = False
        for step, batch in tqdm(enumerate(get_epoch_batches(tokenizer, model_engine.device, val_dataset, micro_batch_size, seed)), desc="testing", leave=False):
        ...
        tqdm.write(f"weighted_bit_mean: {weighted_bit_mean}")
        bit_mean.append(weighted_bit_mean)
        my_logger.info(f"val acc: epoch {epoch+1}:ori_model acc:{orig_acc[epoch+1]}, quantized_model acc:{quant_acc[epoch+1]}, weighted mean: {bit_mean[epoch]}")

...
# 配置自己的 handler
file_handler = logging.FileHandler("sft_bit_mean.txt")
...
class QuantWrapper(nn.Module):
    def __init__(self, model, init_bit=4.0):
        ...
        self.bit = nn.Parameter(torch.load("mix_34_bit_init.pth"), requires_grad=True) 
    ...
    def forward(self, inputs, expect_acc, orig_acc, quant_acc, step_print=False, bit_used=False, grad_print=True):
        ...
        bit_vals = Round.apply(self.bit.view(-1).to(torch.float64))
        ...
        # beta = 0.0 if not bit_used else 0.5
        beta = 0.5
        

        self.loss = sft_loss + beta * weighted_bit_mean
        
        # tqdm.write(f"loss: {self.loss}")
        # if (beta>0):
        #     print(f"bit mean used, loss:{self.loss.item()}")
        # else:
        #     print(f"bit mean not used, loss:{self.loss.item()}")        
...
def main():
    ...
    # weighted_bit_mean = (bit_vals * weights).sum() / weights.sum()
    # print(f"weighted_bit_mean: {weighted_bit_mean}")
    ...
    # bit_tensor = model_engine.module.bit.detach().cpu()
    # torch.save(bit_tensor, "mix_34_bit_init.pth")
    # bit_param = torch.load("mix_34_bit_init.pth")
    ...
    for epoch in range(num_epoches):
        ...
        for step, batch in tqdm(enumerate(get_epoch_batches(tokenizer, model_engine.device, dataset, micro_batch_size, seed)), desc="training", leave=False):
            # batch = get_batch(tokenizer, model_engine.device)
            if(step % 100 == 0):
                step_print = True
            else:
                step_print = False
            ...
            #     print(f"epoch {epoch+1}:ori_model acc:{orig_acc}, quantized_model acc:{quant_acc} ")
            #     orig_corrects = 0
            #     data_size = 0
            #     quant_corrects = 0
            # 为什么batch能到三四千
            # if step % 100 == 0:
            #     bit_grads = model_engine.module.bit_grads
            #     tqdm.write(f"Batch {step} | Loss: {loss.item():.6f} | x: {torch.mean(model_engine.module.bit).item():.6f}")
            #     # my_logger.info(f"Loss: {model_engine.module.loss.item():.6f}")
            #     if bit_grads is not None:
            #         tqdm.write(f"bit.grad.mean: {bit_grads.mean().item():.6f} | norm: {bit_grads.norm().item():.6f}")
            #     else:
            #         tqdm.write("bit.grad is None")
        ...
        tqdm.write(f"epoch {epoch+1}:ori_model acc:{orig_acc[epoch+1]}, quantized_model acc:{quant_acc[epoch+1]} ")
    ...
    # delta_bit = model_engine.module.bit - ori_bit
    # torch.save(model_engine.module.bit, "mix_34_bit_init.pth")
    bit_tensor = model_engine.module.bit.detach().cpu()
    torch.save(bit_tensor, "mix_34_largebeta_bit_init.pth")
    # bit_param = torch.load("mix_34_bit_init.pth")
    # model_engine.module.bit.data.copy_(bit_param)
    for epoch in range(num_epoches):
    ...
    # my_logger.info("Bit comparason")
    # for i in range(len(model_engine.module.bit)):
    #     my_logger.info(f"Forwarding module: linear module{i}, using bit[{i}] = {delta_bit[i].item():.4f}")

...
class QuantWrapper(nn.Module):
    def __init__(self, model, init_bit=4.0):
        ...
        self.qlayer_num = len(self.linear_modules)
        torch.manual_seed(42)
        mask = torch.rand(self.qlayer_num, 1) < 0.5
    ...
    def forward(self, inputs, expect_acc=0, orig_acc=0, quant_acc=0, step_print=False, grad_print=True):
        ...
        def make_patched_forward(i, layer_name):
            def patched_forward(module, input):
                ...
                #     += F.mse_loss(q, ref, reduction='sum')
                return q
        ...
        def make_q_hook():
            ...
            return _hook
        for m in self.linear_modules:
            q_hooks.append(m.register_forward_hook(make_q_hook()))
        ...
        # loss = alpha * kl_loss - beta * self.bit.mean() 
        n_calls_per_layer = len(ref_outs) // self.qlayer_num
        
        for ref, quant in zip(ref_outs, q_outs):
           # 你之前用 reduction='sum'，这里看需要换 mean 还是 sum
            # print(f"per_layer mse:{F.mse_loss(quant, ref, reduction='mean')}")
            # self.layer_loss += F.mse_loss(quant, ref, reduction='mean')
            q_log_p = F.log_softmax(quant, dim=2)
            orig_log_p = F.softmax(ref, dim=2) + 1e-8
            self.layer_loss += F.kl_div(q_log_p, orig_log_p, reduction="none").sum(-1).mean()
        # loss = self.layer_loss / (self.qlayer_num * n_calls_per_layer)
